...
## BattleShip
### BattleShipBase.cpp
플레이어가 조종하는 함선으로 가장 기능이 많이 들어가 있는 주요 클래스로 플레이어가 직접 빙의하지 않고 AI 컨트롤러에 명령하는 구조
...
<br>
#### 컴포넌트
BoxComp(UBoxComponent) : 러프하게 다른 유닛과 거리 비교 등을 수행

SkeletalMeshComp(USkeletalMeshComponent) : 실질적인 충돌과 함포를 배치하기 위한 스켈레톤 구조

SmokestackComp(UNiagaraComponent) : 함선 속도에 따라 연돌의 연기를 제어

BattleShipMovementComp : 직선 운동과 조타를 통한 방향 전환을 하는 함선의 움직임을 위한 컴포넌트
